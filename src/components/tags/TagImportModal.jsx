...
import { REQUIRED_TAG_COLUMNS } from '../../utils/tagHandlers'

function TagImportModal({ isOpen, onClose, onImport, loading = false }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Import Tags" size="sm">
      <div className="space-y-3">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded p-2 text-xs">
          ...
          <div className="text-blue-800 grid grid-cols-2 gap-1">
            {REQUIRED_TAG_COLUMNS.map((col) => (
              <div key={col} className="text-xs">
                ✓ {col}
              </div>
            ))}
          </div>
          <p className="text-blue-700 text-xs mt-1">CSV or Excel format only</p>
        </div>
        ...
        {file && (
          <div className="bg-green-50 border border-green-200 rounded p-2">
            <p className="text-xs text-green-800">✓ {file.name}</p>
          </div>
        )}
